...
import { CenterZoneCalculator, drawZones, handleCanvasClick, handleMouseMovePID } from 'modules/_shared/helpers/traffic-draw-utils';
...
export const WrongWayDetectionSetting: React.FC<WrongWayDetectionSettingProps> = ({
  leftContent,
  osdConfig,
  zones = [],
  currentOrder,
  ...
  useEffect(() => {
    ...
    return () => {
      if (videoElement) {
        console.log("[Wrong Way] Component unmounted, pausing video...");
        videoElement.pause();
      }
    };
  }, [loadData]);
          ...
          <CanvasOverlay
            ...
            onClick={(event) => {
              handleCanvasClick(event, canvasRef, setPoints, onZonesChange, zones, setSelectedZoneIndex, 
                MAX_ZONE, currentOrder, MAX_POINTS, false, currentDirection)}
            }
            onMouseMove={(event) => { if (zones.length < MAX_ZONE) {
              handleMouseMovePID(event, canvasRef, points, zones, false, true)}
            }}
          />
        </VideoFrame>
