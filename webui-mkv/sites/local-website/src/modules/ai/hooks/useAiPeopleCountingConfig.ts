...
export const useAIPeopleCountingConfig = (options?: {
  onSuccess?: (data: IPeopleCountingSetting) => void;
  signal?: AbortSignal;
}) => {
  ...
  const { data, isLoading, error, mutate } = useSWR(
    'system-ai-people',
    () => {
      return aiService.getAiPeopleCountingConfig({ signal: options?.signal });
    },
    {
      onError: (error) => {
  ...
  const updateConfig = async (config: Partial<IPeopleCountingSetting>, retryCount = 0) => {
    try {
      if (!data) return;
      setActionLoading(true);
      const res = await aiService.setAiPeopleCountingConfig({
        ...data,
        ...config,
      });
      ...
      if (error['reason'].includes('AI enable failed')) {
        const newError = Object.assign({}, error, { tab_name: "person_counting" });
