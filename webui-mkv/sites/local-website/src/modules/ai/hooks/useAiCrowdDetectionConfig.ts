...
export const useAICrowdDetectionConfig = (options?: {
  onSuccess?: (data: ICrowdDetectionSetting) => void;
  signal?: AbortSignal;
}) => {
  ...
  const { data = {}, isLoading, error, mutate } = useSWR(
    'system-ai-crow',
    () => {
      return aiService.getAiCrowdDetectionConfig({ signal: options?.signal });
    },
    {
      onError: (error) => {
  ...
  const updateConfig = async (config: Partial<ICrowdDetectionSetting>, retryCount = 0) => {
    try {
      if (!data) return;
      setActionLoading(true);
      const res = await aiService.setAiCrowdDetectionConfig({
        ...data,
        ...config,
      });
      ...
      if (error['reason'].includes('AI enable failed')) {
        const newError = Object.assign({}, error, { tab_name: "crowd_detection" });
