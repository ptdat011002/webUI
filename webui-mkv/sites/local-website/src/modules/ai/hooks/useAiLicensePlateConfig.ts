...
export const useAiLicensePlateConfig = (options?: {
  onSuccess?: (data: ILicensePlateRecognitionSetting) => void;
  signal?: AbortSignal;
}) => {
  const [actionLoading, setActionLoading] = React.useState(false);
  const { handlerError } = useAPIErrorHandler();
  const { data, isLoading, error, mutate } = useSWR(
    'system-ai-license-plate',
    () => {
      return aiService.getAiLicensePlateRecognitionConfig({ signal: options?.signal });
    },
    {
      onError: (error) => {
...
const updateConfig = async (config: Partial<ILicensePlateRecognitionSetting>, retryCount = 0) => {
    try {
      if (!data) return;
      setActionLoading(true);
      const res = await aiService.setAiLicensePlateRecognitionConfig({
        ...data,
        ...config,
      });
      ...
      if (error['reason'].includes('AI enable failed')) {
        const newError = Object.assign({}, error, { tab_name: "license_plate_recognition" });
